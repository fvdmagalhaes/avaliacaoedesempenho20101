...
public class EventoIniciaRecepcao extends Evento{
	 ...
	 * Caso esteja recebendo algum outro quadro, significa que houve uma colisão na rede, mas ela não está envolvida, portanto nada deve ser feito
	 * Caso esteja ocioso ou preparando para transmitir, começará a receber a mensagem normalmente
	 */
	
	@Override
	public ArrayList<Evento> acao(ArrayList<Evento> listaEventos){
		if(SimuladorDebug.isDebbuging())
			SimuladorDebug.escreveLog("EVENTO INICIA RECEPÇÃO OCORREU EM " + this.getTempoInicial() + " NA ESTAÇÃO " + this.getEstacao().getIdentificador()+"\n");
		
		//criando a classe de serviço
		Servicos servicos = new Servicos();
		
		//testa o estado em que se encontra a Estação
		if(this.getEstacao().getEstado() == Estacao.ESTADO_OCIOSO || this.getEstacao().getEstado() == Estacao.ESTADO_PREPARANDO_TRANSFERIR){
